...
object Problem0068 {
  def main(args : Array[String]) : Unit = {
    ...
    nums.foreach( 
      a1 => {
        val numsa1 = nums.filter{ _ != a1 }
        numsa1.foreach { a2 =>
          val numsa2 = numsa1.filter{ _ !=  a2 }
          numsa2.foreach { b1 =>
            val numsb1 = numsa2.filter{ _ !=  b1 }
            numsb1.foreach { b2 =>
              val numsb2 = numsb1.filter{ _ !=  b2 }
              numsb2.foreach { c1 =>
                val numsc1 = numsb2.filter{ _ !=  c1 }
                numsc1.foreach { c2 =>
                  val numsc2 = numsc1.filter{ _ !=  c2 }
                  numsc2.foreach { d1 =>
                    val numsd1 = numsc2.filter{ _ !=  d1 }
                    numsd1.foreach { d2 =>
                      val numsd2 = numsd1.filter{ _ !=  d2 }
                      numsd2.foreach{ e1 =>
                        val numse1 = numsd2.filter{ _ !=  e1 }
                        numse1.foreach{ e2 =>
